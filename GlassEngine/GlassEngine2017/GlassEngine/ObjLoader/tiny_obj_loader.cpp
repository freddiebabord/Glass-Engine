...
void tinyobj::LoadMtl(std::map<std::string, int>* material_map, std::vector<material_t>* materials, std::istream* inStream, std::string* warning)
{
...
bool tinyobj::LoadObj(attrib_t* attrib, std::vector<shape_t>* shapes, std::vector<material_t>* materials, std::string* err, const char* filename, const char* mtl_basedir, bool triangulate)
{
	...
	return LoadObj(attrib, shapes, materials, err, &ifs, &matFileReader, triangulate);
}

bool tinyobj::LoadObj(attrib_t* attrib, std::vector<shape_t>* shapes, std::vector<material_t>* materials, std::string* err, std::istream* inStream, MaterialReader* readMatFn, bool triangulate)
{
	std::stringstream errss;
...
bool tinyobj::LoadObjWithCallback(std::istream& inStream, const callback_t& callback, void* user_data, MaterialReader* readMatFn, std::string* err)
{
	std::stringstream errss;
...
void tinyobj::SplitString(const std::string& s, char delim, std::vector<std::string>& elems)
{
	std::stringstream ss;
	ss.str(s);
	std::string item;
...
bool tinyobj::exportFaceGroupToShape(shape_t* shape, const std::vector<std::vector<vertex_index>>& faceGroup, const std::vector<tag_t>& tags, const int material_id, const std::string& name, bool triangulate)
{
	if (faceGroup.empty())
	{
		return false;
...
std::istream& tinyobj::safeGetline(std::istream& is, std::string& t)
{
	t.clear();
...
int tinyobj::fixIndex(int idx, int n)
{
...
std::string tinyobj::parseString(const char** token)
{
	std::string s;
	(*token) += strspn((*token), " \t");
...
int tinyobj::parseInt(const char** token)
{
	(*token) += strspn((*token), " \t");
	int i = atoi((*token));
...
bool tinyobj::tryParseDouble(const char* s, const char* s_end, double* result)
{
	if (s >= s_end)
	{
		return false;
...
float tinyobj::parseFloat(const char** token, double default_value)
{
	(*token) += strspn((*token), " \t");
...
void tinyobj::parseFloat2(float* x, float* y, const char** token, const double default_x, const double default_y)
{
	(*x) = parseFloat(token, default_x);
	(*y) = parseFloat(token, default_y);
}

void tinyobj::parseFloat3(float* x, float* y, float* z, const char** token, const double default_x, const double default_y, const double default_z)
{
	(*x) = parseFloat(token, default_x);
	(*y) = parseFloat(token, default_y);
...
void tinyobj::parseV(float* x, float* y, float* z, float* w, const char** token, const double default_x, const double default_y, const double default_z, const double default_w)
{
	(*x) = parseFloat(token, default_x);
	(*y) = parseFloat(token, default_y);
	(*z) = parseFloat(token, default_z);
...
bool tinyobj::parseOnOff(const char** token, bool default_value)
{
	(*token) += strspn((*token), " \t");
	const char* end = (*token) + strcspn((*token), " \t\r");
...
texture_type_t tinyobj::parseTextureType(const char** token, texture_type_t default_value)
{
	(*token) += strspn((*token), " \t");
	const char* end = (*token) + strcspn((*token), " \t\r");
...
tag_sizes tinyobj::parseTagTriple(const char** token)
{
	tag_sizes ts;
...
vertex_index tinyobj::parseTriple(const char** token, int vsize, int vnsize, int vtsize)
{
	vertex_index vi(-1);
...
vertex_index tinyobj::parseRawTriple(const char** token)
{
